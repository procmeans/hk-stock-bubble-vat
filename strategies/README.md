# strategies — 经典量化策略 MVP

9 个经典策略的最小实现,统一回测口径,教学/研究用途。
设计文档:`docs/plans/2026-07-11-strategies-mvp-design.md`
(分层概念对齐 zipline/pyfolio:data=bundle,signal=algorithm,
backtest=execution+commission/slippage,metrics=tear sheet)。

## 用法

    # 先准备任一市场缓存
    python -m alpha101.yf_history fetch --market us
    # 单策略 / 全部对比
    python -m strategies.run --market us --strategy momentum
    python -m strategies.run --market us --all

策略:ma_cross(双均线)、mean_reversion(均值回归)、momentum(动量)、
market_neutral(市场中性多空)、pairs(配对套利)、ml(滚动逻辑回归)、
elastic_net(弹性网络选股)、icir_weight(ICIR 定权多因子)、
lasso(Lasso 滚动选股)——后三个参照 BigQuant 文档,基本面因子以
价量因子替代(面板无 point-in-time 基本面)。

## 有效性验证与调参

    python -m strategies.validate --market us --all                # 基准/超额/t检验
    python -m strategies.validate --market us --strategy momentum  # 加分年表
    python -m strategies.optimize --market us --strategy ma_cross  # 训练/留出调参

结论口径:t≥2 显著跑赢;|t|<2 超额不显著(不能排除运气);
优化的留出段只用一次,红牌参数勿采用。已退市股缺失 = 残余幸存者偏差。

## 模拟交易(paper trading)

美股 momentum(lookback=126/top_n=40,唯一通过显著性验证的策略)的前向纸面实盘,
GitHub Actions 每日美股收盘后自动步进(`.github/workflows/paper.yml`),
状态在 `paper/us_momentum/`(state.json / nav.csv / orders.csv),全程可审计:

    python -m strategies.paper init --capital 100000   # 建账户(一次性)
    python -m strategies.paper run                     # 每日步进(Actions 自动)
    python -m strategies.paper status                  # 查看持仓与净值

口径:T 日收盘信号 → T+1 收盘成交,单边 20bp;`nav.csv` 同时记等权基准,
若干个月后与回测预期对照,是策略的样本外终审。

多账户:`paper/accounts.json` 是账户清单(页面据此渲染切换)。
A 股账户由 `paper-a.yml` 每日 UTC 07:30 统一步进(`run-market --market a`,
同市场共用一次 iFinD 抓数):

- `a_alpha101`:alpha101 全因子组合,top 50、每 5 日调仓——4 年历史检验
  显著跑输等权基准(t=-4.2),上盘属延长检验;
- `a_equal_weight`:流动性前 500 等权、每 5 日再平衡——近 4 年事实冠军
  风格,风险为小微盘拥挤与雪崩式回撤。

A 股纸面模拟忽略整手与涨跌停约束。

口径:T 日收盘信号次日生效;换手计单边成本(默认 20bp);
A 股做空为纸面模拟。研究用途,不构成投资建议。

同花顺热度双账户由同一 A 股工作流步进：

- `a_ths_heat`：指定交易日个股热度 top 20；
- `a_ths_heat_rise`：指定交易日个股热度排名环比增长率 top 20。

两者均等权、每 2 个交易日调仓、T 日信号次日收盘成交、单边 20bp。
实际使用的信号写入 `paper/ths_heat_signals.csv`。热度代表关注度而非经过验证的
正向 alpha；高关注可能来自价格过度反应，且热度上升组合可能产生较高换手。

## 同花顺注意力三因子组合双账户

两个账户共用指定 T 日的“个股热度排名环比增长率” Top 100 候选池，
并同时取 T 日流通 A 股与总股本。入池后先排除 `ST` / `*ST` 股；未前向
填充的日收盘价中必须至少有 60 个有限正数观测，且当日 T 与精确 T-7
交易日收盘价都必须为有限正数。注意力上升、流通 A 股和总股本也必须
有限，且股本为正、流通比例不超过 1。

因子公式为：

    attention_rise = 个股热度排名环比增长率
    momentum_7d = close[T] / close[T-7个交易日] - 1
    float_ratio = 流通A股 / 总股本

分位数只在共同有效候选池内计算：`attention_pct` 和 `momentum_pct`
按升序百分位排名，`low_float_pct` 按流通比例降序百分位排名；
并列值取平均百分位，三者都是分数越高越好。

`a_ths_attention_weighted` 使用精确加权分数：

    score = 0.50 * attention_pct + 0.30 * momentum_pct + 0.20 * low_float_pct

按分数、注意力上升、股票代码稳定排序后取 Top 20 等权；最终有 `N` 只时
每只目标权重为 `1 / N`。
`a_ths_attention_funnel` 则从同一 Top 100 候选池逐层筛选：7 日动量为正
→ 流通比例最低的前 50%(向上取整) → 按注意力上升排序取 Top 20 等权。

两者均每 2 个交易日调仓：T 日收盘后生成信号，T+1 收盘交易，单边成本
20bp。实际使用的选股与错误审计写入
`paper/ths_attention_combo_signals.csv`。候选查询失败时两个注意力账户
暂停新挂单，单账户筛选失败不影响另一账户或普通 A 股账户；到期失败
不重置调仓计数，下一交易日自动重试。

注意力、短期动量和低流通比例均可能放大拥挤与波动；两个账户只比较组合方法，
不代表三因子或任一权重已经通过历史显著性验证。
